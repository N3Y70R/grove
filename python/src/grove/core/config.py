...
# Name of the policy profile the repo was created with (recorded in grove.toml
...
def _anchored_ticket_re(pattern: str) -> "re.Pattern[str]":
    """Compile a ticket pattern anchored at token boundaries, so a version is not
    read as a ticket ('fix/python-0.6.1' must not yield PYTHON-0): no alphanumeric
    right before the key, and no digit or '.<digit>' right after it."""
    return re.compile(rf"(?<![A-Za-z0-9])(?:{pattern})(?![0-9]|\.[0-9])", re.IGNORECASE)


TICKET_RE = _anchored_ticket_re(TICKET_PATTERN)
...
def _compute_ticket_re(pattern: str) -> None:
    ...
    TICKET_RE = _anchored_ticket_re(pattern)
...
def load(bare: Path) -> None:
    ...
    if cfg.is_file():
        data = _read_toml(cfg)
        # Layer the repo's own profile first, so a key missing from grove.toml
        # (e.g. after `config unset`) falls back to the profile the repo was
        # created with — not to grove's internal work-style defaults.
        name = data.get("profile") or DEFAULT_PROFILE
        try:
            base_policy = resolve_profile(name)
        except KeyError:  # profile no longer defined (e.g. removed from config.toml)
            base_policy = resolve_profile(DEFAULT_PROFILE)
...
def render_repo_config(policy: dict) -> str:
    ...
    lines = ["# grove configuration for this repo (generated by 'gwt setup' / 'gwt convert').", ""]
    for key in ("profile", "parking_branch", "default_base", "allowed_types",
