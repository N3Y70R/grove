...
### 6.15b2 `gwt skill status`

Machine-level report of the user-level skill copies (`~/.agents/skills/grove`, `~/.claude/skills/grove`): `version` (running), `skills` (the same rows as `doctor`'s `skills`), `hint` (next step, or null when every copy is current and untouched). Works outside any repo. MCP: `grove_skill_status`.
